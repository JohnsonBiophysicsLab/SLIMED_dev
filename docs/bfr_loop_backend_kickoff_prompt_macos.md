# Copy-ready kickoff prompt: Bfr Loop backend, B0a, B0b, B0c, B1 (macOS)
...
repository at ~/Workspace/SLIMED_dev.

Read these before any action, in order:
  docs/bfr_loop_backend_plan_macos.md (this lane; B0a-B4, L1-L8, D9a-D11)
...
The authoritative profile is the supported macOS environment and `python3`,
...
runs from the repository root in a macOS terminal:
  cd ~/Workspace/SLIMED_dev
and uses `python3`, never `python`. If a command is run anywhere else, report it
as an unofficial reproduction and re-run it on macOS from the repository root
before claiming evidence. Record `sw_vers -productVersion`, `uname -m`, and
`python3 --version` once, in the first status record, so reviewers can reproduce
the same environment and interpreter.
...
DECISIONS ALREADY MADE BY THE USER - DO NOT RE-ASK
  ...
  See docs/bfr_loop_backend_plan_macos.md section 2.0. If a reviewer disputes that
...
Create the branch yourself, then dispatch ONE implementation subagent:
  ...
  Implement B0a from docs/bfr_loop_backend_plan_macos.md section 5. Enforce C1, C3,
  C5, C7, P3, P8, S5.
  ...
  Required evidence, on macOS from the repository root, all commands and output
  verbatim:
...
gatekeeping. Use exactly this structure:
  ...
  Rules to enforce: C1, C3, C5, C7, P3, P4, P8, S5, and that package's gate in
    docs/bfr_loop_backend_plan_macos.md
  Required commands: that package's evidence commands, run on macOS from the
...
workflow under .github/workflows/ and docs/bfr_loop_backend_plan_macos.md, and nothing
...
docs/bfr_loop_backend_plan_macos.md. The recorded status must carry the scope limits
...
STATUS RECORD - post after every subagent or reviewer response
  ...
  Environment: <macOS version, architecture, python3 --version>  (first record only)
