...
clearing pre-pass implementation placement, geometry-refresh extraction, and
...
geometry boundary, not as an evaluator-phase extraction. The documented
positive-depth `nOneRingVertices == 11` `4+3+4` case now routes through
subdivision-transpose back-projection instead of falling through to
`element_energy_force_regular(...)`; zero-depth 11-control requests and
unsupported topologies still fail before the membrane force loop. Future work
should decide whether a unified backend such as OpenSubdiv is preferable before
broadening support beyond this focused in-repo matrix route.

The current characterization map is
`docs/irregular_patch_fixture_requirements.md`. It adds 11-control
area/volume fixture coverage, a positive-depth subdivision-transpose force
route, and unsupported-route guard coverage. Broader irregular support remains
a known gap until a wider backend/valence contract is approved.
...
| --- | --- | --- | --- | --- | --- |
| Keep the shared helper and move implementation work behind `EnergyForceEvaluator` | Partly implemented: the clearing pre-pass, geometry refresh, and membrane face-loop force reduction are now named inside the existing implementation boundary. Remaining slices should avoid cross-mode coupling while enabling internal physics extraction behind the existing facade. | Medium: internal extraction can disturb formula order, force scatter, OpenMP reductions, scaffolding side effects, or boundary force handling. | `src/energy_force/*`, `include/energy_force/*`, focused tests/docs. | `make test`, `./bin/test_main`, evaluator equivalence tests, OpenMP/serial numerical baseline, accepted-step smoke if runner behavior is touched. | Yes, because production physics code changes. |
| Extract actual membrane term accumulation after clearing | Implemented narrowly as `accumulate_membrane_face_energy_and_forces(mesh)`, a private translation-unit helper that moves the existing face loop together with its thread-local force scatter reduction. | Still high for any further split: formula internals, quadrature order, one-ring coordinate capture, regular/irregular path selection, per-thread force accumulation, and reduction into vertex force components remain tightly coupled. | `src/energy_force/Compute_energy_and_force_on_mesh.cpp`, focused docs/scripts. | Fresh serial/OpenMP numerical baseline, evaluator equivalence tests, force-component comparisons, boundary/ghost checks, scaffold-disabled and scaffold-enabled checks, accepted-step smoke if timing moves. | Yes, explicit reviewer/domain approval recommended before further splitting. |
