...
| Broader-valence coverage | Representative synthetic extraordinary-valence classes report derivative-complete source-id visibility, and the serialized octahedron stand-in has an approved proof-only 24-sample Ptex/source-row/linear-transpose report. Production broader-valence support still needs membrane outputs, actual production formula/scatter/OpenMP evidence, generic scientific signoff, and route review. |
