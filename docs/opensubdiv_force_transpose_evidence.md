...
| Approved valence-4 mapping/sample/transpose proof | `scripts/run_irregular_valence4_opensubdiv_mapping_proof.sh` invokes `--valence4-mapping-sample-transpose-report`. | The serialized octahedron's eight oriented faces map to Ptex IDs `0..7`; 24 frozen samples emit dense seven-by-six rows, patch-basis/limit-stencil agreement, deterministic duplicate aggregation, source coverage unions, and per-sample/stacked linear transpose evidence. | It does not prove production geometry, fBend/fArea/fVolume, scatter/OpenMP parity, generic scientific validity, or route readiness. |
| Approved proof-only valence-4 force-formula lane | `scripts/run_irregular_valence4_opensubdiv_force_formula_proof.sh` runs the existing regular production-helper bridge before `--valence4-force-formula-proof-report`. | On a deterministic in-memory asymmetric perturbation, all 24 frozen face-samples produce finite proof-local geometry, energies, and nonzero source-keyed `fBend`/`fArea`/`fVolume`; all 54 source-axis force comparisons pass an independent scalar-energy centered-difference sweep, with duplicate aggregation, rigid-motion residuals, and byte-identical repeat evidence. | It is not physics validation. Its full-dot volume scalar is not the legacy visible-volume output; it does not prove production geometry/output parity, `Face::oneRingVertices` scatter, serial/OpenMP parity, route readiness, broader-valence routing, or scientific equivalence. |
| Proof-only valence-4 scatter/OpenMP shape lane | `scripts/run_irregular_valence4_opensubdiv_scatter_openmp_proof.sh` gates the PR #117 force report's `production_scatter_openmp_shape_proof`. | Eight finite/nonzero face-local contributions scatter through original source IDs into the reviewed nine-component buffer shape; a distinct-sentinel exact-index oracle independently checks all 54 destinations, all six sources receive multi-face collisions, direct and split/reversed-duplicate aggregation agree, and serial accumulation matches a three-buffer simulated OpenMP-style reduction under `1e-12`. | It does not populate production `Face::oneRingVertices`, invoke an OpenMP runtime, compare executables, enable routing, or validate physics/output semantics. |
