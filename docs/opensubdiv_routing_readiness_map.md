...
Date: 2026-07-17.
Baseline: post-PR93 regular OpenSubdiv route-parity resolution diagnostics plus
this row precision/tolerance readiness recheck.
...
- `diagnose_opensubdiv_regular_production_call_parity` can be called only as a
  ...
  `routedResidualActivationBlocker` so the current state is recorded as a
