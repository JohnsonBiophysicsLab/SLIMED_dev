...
Date: 2026-06-30.
Baseline: post-PR85 regular OpenSubdiv row-semantics evidence plus this
diagnostic-only production-call parity recheck.
...
- `diagnose_opensubdiv_regular_production_call_parity` can be called only as a
  ...
  `fVolume`, and scatter-buffer deltas, including the face/local-row/axis or
  vertex/component location of the largest residual plus the signed
  ...
  including the face, sample, derivative row, source column, and signed
  direct/routed row values at the largest row residual, so the remaining
  force/scatter residual can be read against the row-error budget.
...
| Scatter through `Face::oneRingVertices` | `RegularForceRowsScatterInOneRingOrder`, the C++ proof harness, and the diagnostic-only production-call parity recheck use the reviewed one-ring order. The current recheck reports residual scatter-buffer deltas caused by the remaining force-row mismatch and records the signed direct/routed values at the largest scatter residual. | The first routed production PR must preserve that scatter order or ship with a reviewed replacement order and tests. |
