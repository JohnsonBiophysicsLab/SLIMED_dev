...
- `diagnose_opensubdiv_regular_production_call_parity` can be called only as a
  ...
  weight difference against the frozen SLIMED rows for the same recheck so the
  remaining force/scatter residual can be read against the row-error budget.
