...
- `diagnose_opensubdiv_regular_production_call_parity` can be called only as a
  ...
  amplification ratios against the maximum row-weight residual. This isolates
  the remaining blocker to OpenSubdiv row precision or a reviewed
  tolerance/precision decision rather than an obvious `Face::oneRingVertices`
  scatter wiring bug.
