...
def execution_manifest() -> dict[str, Any]:
    ...
    opensubdiv_common_cmake_options = [
        "-G", "Unix Makefiles",
        ...
        "-DCMAKE_AR=/Library/Developer/CommandLineTools/usr/bin/ar",
        "-DCMAKE_RANLIB=/Library/Developer/CommandLineTools/usr/bin/ranlib",
        "-DCMAKE_MAKE_PROGRAM=/usr/bin/make",
        "-DCMAKE_INSTALL_PREFIX=${PROFILE_INSTALL_ROOT}",
    ...
    return {
        ...
        "qualification_platform": {
            "build": {
                ...
                "opensubdiv": {
                    "archive_target": "osd_static_cpu",
                    "build_environment": {"LANG": "C", "LC_ALL": "C", "SOURCE_DATE_EPOCH": "0", "TZ": "UTC", "ZERO_AR_DATE": "1"},
                    "build_tool": {"path": "/usr/bin/make", "version": "GNU Make 3.81"},
                    "cmake": {"common_options_in_exact_order": opensubdiv_common_cmake_options, "generator": "Unix Makefiles", "path": "/opt/homebrew/bin/cmake", "version": "4.4.2"},
                    "commands": {
                        "build": "env <exact build_environment> /opt/homebrew/bin/cmake --build ${PROFILE_BUILD_ROOT} --config Release --target osd_static_cpu --parallel 1 --verbose",
                        "configure": "env <exact build_environment> /opt/homebrew/bin/cmake -S ${OPENSUBDIV_SOURCE} -B ${PROFILE_BUILD_ROOT} <common_options_in_exact_order> -DCMAKE_CXX_FLAGS= -DCMAKE_CXX_FLAGS_RELEASE=<profile flags joined by single spaces>",
                    ...
                    "tag_commit": "9dab8a47bfbb1388ec8388fe61f5f916e6123f38",
                    "translation_units_in_target_order": opensubdiv_translation_units,
                    "version": "3.7.0",
                },
                "oracle_separation": "the D10 MPFR oracle remains a separate executable and may not link OpenSubdiv",
                "thread_sanitizer_compile_flags": tsan_flags,
                "thread_sanitizer_include_flags": ["-I${OPENSUBDIV_TSAN_ROOT}/include"],
                "thread_sanitizer_link_inputs": ["-fsanitize=thread", "${OPENSUBDIV_TSAN_ROOT}/lib/libosdCPU.a", "-framework", "IOKit", "-framework", "Foundation"],
                "thread_sanitizer_opensubdiv_requirement": "build exactly osd_static_cpu's frozen 47-translation-unit scope from the clean pinned source with the thread_sanitizer profile into the exact disjoint OPENSUBDIV_TSAN_ROOT archive and link that instrumented archive only",
            },
            ...
            "power": {"api": "IOPSCopyPowerSourcesInfo plus IOPSGetProvidingPowerSourceType", "required_value": "kIOPSACPowerValue", "sampling": "before and after every full case process"},
        ...
        "sample_field_contract": {
            ...
            "weight": {"binary64_hex": "0x1.0000000000000p+0", "bits_hex": "3ff0000000000000", "b1_validation": "strictly positive and finite", "meaning": "validation-only sentinel; forbidden for quadrature, integration, or any D9b claim"},
        },
        "sample_policies": [
