...
{
    if (mesh.vertices.size() != kReviewedSourceCount ||
        rows.size() != mesh.faces.size())
    {
        throw std::invalid_argument(
            "valence-3 Phase 3 requires complete four-source row coverage");
    }

    std::vector<SourceMappingView> mappings;
    mappings.reserve(rows.size());
    const std::vector<int> expectedSources{0, 1, 2, 3};
    for (std::size_t faceIndex = 0; faceIndex < rows.size(); ++faceIndex)
    {
        ...
        {
            throw std::invalid_argument(
                "valence-3 Phase 3 rejected face, source, sample, or "
                "one-ring drift");
        }
        ...
        mapping.originalSourceIds = expectedSources;
    ...
    const std::vector<int> &sourceIds)
{
    ...
    for (const int sourceId : sourceIds)
    {
        if (sourceId < 0 ||
            ...
            !matrix_is_finite(mesh.vertices[sourceId].coord))
        {
            throw std::invalid_argument(
                "valence-3 Phase 3 rejected source coordinate drift");
        }
    ...
    const PreparedSourceKeyedFace &face)
{
    ...
    for (const auto &sample : face.samples)
    {
        Matrix rows(source_keyed_kernel::kDerivativeRowCount,
                    static_cast<int>(sourceIds.size()), true);
        for (int rowIndex = 0;
             ...
             ++rowIndex)
        {
            ...
            {
                throw std::invalid_argument(
                    "valence-3 Phase 3 rejected row/source mapping drift");
            }
            for (std::size_t source = 0; source < sourceIds.size(); ++source)
            {
                rows.set(rowIndex, static_cast<int>(source),
    ...
    const PreparedSourceKeyedFace &face)
{
    ...
    {
        throw std::invalid_argument(
            "valence-3 Phase 3 geometry rejected quadrature drift");
    }

    for (std::size_t sampleIndex = 0;
         sampleIndex < face.samples.size(); ++sampleIndex)
    {
        std::array<Vec3, 3> evaluated{};
        for (int rowIndex = 0; rowIndex < 3; ++rowIndex)
        {
            const auto &row = face.samples[sampleIndex].rows[rowIndex];
            for (std::size_t source = 0; source < coordinates.size(); ++source)
            {
                for (int axis = 0;
                     axis < source_keyed_kernel::kAxisCount; ++axis)
                {
    ...
    {
        throw std::invalid_argument(
            "valence-3 Phase 3 geometry produced invalid output");
    }
    return geometry;
}
...
{
    std::vector<Valence3Phase3FaceObservables> observables;
    ...
    const double totalVolume)
{
    ...
    for (const PreparedSourceKeyedFace &preparedFace : prepared.faces)
    {
        ...
        evaluator.element_energy_force_regular(
            coordinates, formulaFace, formulaFace.spontCurvature,
            meanCurvature, normal, bendingEnergy,
            bending, area, volume, false, &shapeFunctions);
        ...
        {
            throw std::invalid_argument(
                "valence-3 Phase 3 scientific dry run produced nonfinite "
                "output");
        }

        Valence3Phase3FaceObservables observable;
        ...
        {
            for (int kind = 0;
                 kind < source_keyed_kernel::kForceKindCount; ++kind)
            {
                for (int axis = 0;
                     axis < source_keyed_kernel::kAxisCount; ++axis)
                {
...
double maximum_face_observable_difference(
    const Mesh &mesh,
    const std::vector<Valence3Phase3FaceObservables> &expected)
{
...
{
    const char *value = std::getenv(kPhase3RuntimeOptIn);
    ...
    const Valence3Phase3Request &request)
{
    ...
    if (!result.explicitRequestReceived)
    {
        result.rejectionReason =
            "valence-3 Phase 3 remains default-off without an explicit "
            "scientific-baseline request";
        return result;
    }
    ...
    {
        result.rejectionReason =
            "valence-3 Phase 3 requires the exact ordered N=2 sample plan";
        return result;
    }
    for (int sample = 0;
         sample < static_cast<int>(kReviewedSampleCount); ++sample)
    {
        for (int coordinate = 0; coordinate < 3; ++coordinate)
        {
            ...
            {
                result.rejectionReason =
                    "valence-3 Phase 3 rejected ordered sample drift";
                return result;
            }
        }
    }
    ...
    {
        result.rejectionReason =
            "valence-3 Phase 3 requires three reviewed quadrature weights";
        return result;
    }
    for (int sample = 0;
         sample < static_cast<int>(kReviewedSampleCount); ++sample)
    {
        ...
        {
            result.rejectionReason =
                "valence-3 Phase 3 rejected quadrature weight drift";
            return result;
        }
    }
    ...
    opensubdiv_valence3::OpenSubdivValence3RowProviderRequest rowRequest;
    rowRequest.phase1ProviderExplicitRequest = true;
    result.rowProvider =
        opensubdiv_valence3::build_guarded_opensubdiv_valence3_rows(
            mesh, rowRequest);
    ...
    if (result.rowProvider.opensubdivVersionNumber !=
            kReviewedOpenSubdivVersion ||
        result.rowProvider.adaptiveIsolationLevel !=
            kReviewedIsolationLevel ||
        !result.rowProvider.exactTopologyIdentityValidated ||
        !result.rowProvider.ptexFaceIdentityValidated ||
        !result.rowProvider.exactFourSourceBoundaryValidated)
    {
        result.rejectionReason =
            "valence-3 Phase 3 rejected baseline identity drift";
        return result;
    }
    result.exactBaselineIdentityValidated = true;
    ...
    try
    {
        ...
        {
            throw std::invalid_argument(
                "valence-3 Phase 3 geometry produced invalid global output");
        }
        ...
        !result.outputStateFinite)
    {
        throw std::runtime_error(
            "valence-3 Phase 3 production postcondition failed");
    }
