...
def load_inventory_module():
    spec = importlib.util.spec_from_file_location(
        "inventory_irregular_valence4_atomic_face_loop_publication",
        INVENTORY,
    )
...
class ValenceFourAtomicFaceLoopPublicationInventoryTest(unittest.TestCase):
    def test_inventory_passes_without_real_face_loop_caller(self):
        ...
        self.assertTrue(report["atomic_face_loop_publication_executed"])
    ...
    def test_present_dependency_atomic_publication_passes(self):
        result = subprocess.run(
